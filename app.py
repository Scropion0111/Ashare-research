...
def render_nav_tabs():
    """
    纯横向导航栏 - 基于 query_params + session_state
    使用 Streamlit rerun 机制，避免页面刷新丢失状态
    """
    tabs = [
        (0, "📊", "信号清单"),
        (1, "📈", "行情视图"),
    ...
    # 获取当前 tab（优先使用 session_state，兼容 URL）
    ...
    for idx, icon, name in tabs:
        ...
        tabs_html += f'''<div class="nav-btn {active_class}" id="nav_tab_{idx}" 
            onclick="switchTab({idx})"><span class="nav-icon">{icon}</span>{name}</div>'''
    tabs_html += '</div></div>'
    
    st.markdown(tabs_html, unsafe_allow_html=True)
    
    # 使用 JavaScript 通知 Streamlit 切换 tab
    js_code = '''
    <script>
    function switchTab(idx) {
        // 设置 sessionStorage（跨标签页）
        sessionStorage.setItem('target_tab', idx);
        // 通知 Streamlit
        window.parent.postMessage({type: 'switchTab', tab: idx}, '*');
    }
    </script>
    '''
...
def page_signal_list(key_mask: str):
    ...
    # ========== 法务威慑 ==========
    st.markdown("""
    <div style="font-size:0.7em; color:#9ca3af; margin-top:16px; text-align:center;">
        本内容仅供个人研究与学习使用，禁止转售、二次分发或任何形式的公开传播。<br>
        严禁任何形式的二次收费、转售或商业化使用。<br>
        如发现违规行为，访问授权可能被立即终止，恕不另行通知。
    </div>
    """, unsafe_allow_html=True)
...
def main():
    ...
    横向导航栏 + Key 验证系统
    - 信号清单：需要 Key 验证
    - 行情视图：需要 Key 验证（已验证用户可直接访问）
    - 支持订阅：无需验证，开放访问
    """
    # 处理导航状态
    current_tab = handle_tab_switch()
    st.session_state.target_tab = current_tab
