...
def render_nav_tabs():
    ...
    # 渲染自定义样式的导航栏（用 CSS 把原生 radio 藏起来）
    st.markdown('''
    <style>
    /* 隐藏原生 radio */
    .nav-radio {
    ...
    for idx, icon, name in tabs:
        ...
        tabs_html += f'''
        <label class="nav-custom {active_class}" for="nav_radio_{idx}">
    ...
    for idx, _, _ in tabs:
        ...
        tabs_html += f'<input type="radio" name="nav_radio" id="nav_radio_{idx}" value="{idx}" class="nav-radio" {checked}>'
    tabs_html += '</div>'
