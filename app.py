...
def render_nav_tabs():
    ...
    # 渲染自定义样式的导航栏（用 CSS 把原生 radio 彻底藏起来）
    st.markdown('''
    <style>
    /* ===== 彻底隐藏原生 radio ===== */
    ...
    for idx, icon, name in tabs:
        ...
        tabs_html += f'''
        <label class="nav-btn {active_class}" for="nav_radio_{idx}">
    ...
    for idx, _, _ in tabs:
        ...
        tabs_html += f'<input type="radio" name="nav_radio" id="nav_radio_{idx}" value="{idx}" {checked}>'
    tabs_html += '</div>'
    
    st.markdown(tabs_html, unsafe_allow_html=True)
    
    # 隐藏 st.radio 组件（使用更彻底的方法）
    st.markdown('''
    <style>
    /* 彻底隐藏 radio 相关的所有元素 */
    div[data-testid="stRadio"],
